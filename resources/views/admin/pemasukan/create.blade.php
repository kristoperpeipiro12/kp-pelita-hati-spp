                    ...
                    <form action="{{ route('pemasukan.store') }}" method="POST" class="d-flex flex-column" id="form-pemasukan">
                        ...
                        <div class="hpem-con-form-group">
                            <div class="form-group row">
                                ...
                                <div class="col-12">
                                    <select class="form-control js-select2" name="nis" id="nis">
                                        ...
                                        @endphp
                                        @foreach ($siswa as $p)
                                        <option value="{{ $p->nis }}" {{ old('nis') == $p->nis ? 'selected' : '' }}>{{ $p->nis }} - {{ $p->nama }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="form-group">
                                ...
                                <input type="date" class="form-control" name="tanggal_bayar" value="{{ old('tanggal_bayar', date('Y-m-d')) }}" id="tanggal_bayar">
                            </div>
                        </div>
                                ...
                                <div class="hpem-con-form-group">
                                    <div class="col-lg-4 p-0">
                                        <select class="form-control" name="bulan_tagihan" required>
                                            ...
                                            <option value="{{ $bulan }}" {{ old('bulan_tagihan') == $bulan ? 'selected' : '' }}>{{ $nama }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-lg-4 p-0">
                                        <select class="form-control" name="tahun_tagihan" required>
                                            ...
                                            <option value="{{ $tahun }}" {{ old('tahun_tagihan') == $tahun ? 'selected' : '' }}>{{ $tahun }}</option>
                                            @endfor
                                        </select>
                                    </div>
                                </div>
                        ...
                        <div class="hpem-con-form-group">
                            <div class="form-group w-100">
                                ...
                                <select class="form-control" name="jenis_transaksi" required>
                                    ...
                                    <option value="Kontan" {{ old('jenis_transaksi') == 'Kontan' ? 'selected' : '' }}>Kontan</option>
                                    <option value="Transfer" {{ old('jenis_transaksi') == 'Transfer' ? 'selected' : '' }}>Transfer</option>
                                </select>
                            </div>
                        </div>

                        <div class="w-50 d-flex align-self-end">
                            <button type="submit" class="btn btn-success flex-fill hpem-btn-create w-25">Simpan</button>
